...
interface DrawResult {
  drawId: string; winnerUserId: string; winnerNickname: string; totalEntries: number
}
...
export default function AdminDrawsPage() {
  ...
  useEffect(() => {
    ...
    const h = onValue(r, (snap) => {
      if (snap.exists()) setProducts(Object.values(snap.val() as Record<string, ProductRoom>))
      setLoading(false)
    })
  ...
  async function triggerDraw(roomId: string, productName: string) {
    if (!confirm(`"${productName}" 추첨을 실행하시겠습니까?\n공정한 난수로 당첨자를 선정합니다.`)) return
    setDrawing(roomId); setError(null)
    try {
      ...
      setError((e as { message?: string })?.message ?? '추첨 실패')
  ...
  return (
    <div className="space-y-5">
      ...
      {error && <div className="card border border-red-500/30 bg-red-500/10"><p className="text-sm text-red-400">{error}</p></div>}
      {loading ? (
        <div className="space-y-3">{Array.from({ length: 3 }).map((_, i) => <div key={i} className="card h-28 shimmer" />)}</div>
      ) : (
        <>
              ...
              : openRooms.map((p) => (
                <div key={p.roomId} className="card space-y-3">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <button onClick={() => triggerDraw(p.roomId, p.productName)} disabled={drawing === p.roomId}
                  ...
                  <p className="text-xs text-gray-500">{p.currentCount >= p.capacity ? '✓ 정원 달성 — 즉시 추첨 가능' : `${p.capacity - p.currentCount}명 부족 (강제 추첨 가능)`}</p>
                </div>
              ))}
          </div>
            ...
            <div className="space-y-2">
              ...
              {drawingRooms.map((p) => (
                <div key={p.roomId} className="card border border-yellow-500/30 animate-pulse">
                  <div className="flex items-center gap-2"><GradeBadge grade={p.grade} /><p className="text-sm text-white">{p.productName}</p><span className="ml-auto text-xs text-yellow-400 font-semibold">추첨중...</span></div>
                </div>
              ))}
            </div>
          )}
            ...
            return (
              <div key={roomId} className="card border border-green-500/30 bg-green-500/5 space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => setResults(prev => { const n = { ...prev }; delete n[roomId]; return n })} className="text-xs text-gray-500 hover:text-white">닫기</button>
                </div>
                ...
                <div className="bg-card-lt rounded-xl p-3 space-y-1">
                  ...
                  <p className="text-xs text-gray-500">전체 {result.totalEntries}명 중 선정</p>
                </div>
                <p className="text-xs text-gray-400">FCM 알림 발송 완료. 수령 관리 탭에서 처리하세요.</p>
              </div>
            )
          })}
          {drawnRooms.length > 0 && (
            <div className="space-y-2">
              ...
              {drawnRooms.map((p) => (
                <div key={p.roomId} className="card opacity-60">
                  <div className="flex items-center gap-2"><GradeBadge grade={p.grade} /><p className="text-sm text-white">{p.productName}</p><span className="ml-auto text-xs text-green-400 font-semibold">완료</span></div>
                </div>
              ))}
            </div>
          )}
