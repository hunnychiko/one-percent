...
export const enterMatchQueue = functions.https.onCall(async (data) => {
  ...
  if (!userId || !roomId) {
    throw new functions.https.HttpsError('invalid-argument', 'userId, roomId 필수')
  }
  ...
  snap.forEach(child => {
    if (child.key !== userId && !opponentId) opponentId = child.key
    return false
  })
  ...
  if (opponentId) {
    ...
    await db.ref(`matches/${matchId}`).set({
      matchId, player1: opponentId, player2: userId,
      roomId, streak, createdAt: admin.database.ServerValue.TIMESTAMP
    })

    await sendToUser(opponentId, {
      title: '매칭 완료!',
      body: `${streak}연승 상대를 찾았습니다. 가위바위보를 시작하세요!`,
      data: { type: 'match_found', matchId, roomId }
    })
...
export const executeDraw = functions.https.onCall(async (data) => {
  ...
  const winnerNickname = user?.nickname ?? '알 수 없음'
  ...
  await Promise.all([
    ...
    db.ref(`winnerClaims/${drawId}`).set({
      ...
      productType:        room.productType ?? 'coupon',
      status:             'unclaimed',
      couponCode:         '',
      affiliateUrl:       '',
      shippingName:       '',
  ...
  await sendToUser(winner.userId, {
    title: '🎉 당첨되셨습니다!',
    body:  `${room.productName} 추첨에 당첨되셨습니다. 마이페이지 → 수령하기를 눌러주세요!`,
...
export const issueCoupon = functions.https.onCall(async (data) => {
  const { claimId, couponCode, affiliateUrl } = data as {
    claimId: string; couponCode?: string; affiliateUrl?: string
  }
  if (!claimId) throw new functions.https.HttpsError('invalid-argument', 'claimId 필수')

  const claimSnap = await db.ref(`winnerClaims/${claimId}`).once('value')
  const claim     = claimSnap.val()
  if (!claim) throw new functions.https.HttpsError('not-found', '수령 정보 없음')

  await db.ref(`winnerClaims/${claimId}`).update({
  ...
  await sendToUser(claim.userId, {
    title: '쿠폰이 도착했습니다!',
    body:  `${claim.productName} 쿠폰이 발급됐습니다. 지금 확인하세요!`,
    data:  { type: 'coupon_issued', claimId, roomId: claim.roomId }
  })
...
export const updateShipping = functions.https.onCall(async (data) => {
  const { claimId, trackingNumber, trackingCarrier, status } = data as {
    claimId: string; trackingNumber?: string; trackingCarrier?: string; status?: string
  }
  if (!claimId) throw new functions.https.HttpsError('invalid-argument', 'claimId 필수')
  ...
  if (status === 'shipped') {
    await sendToUser(claim.userId, {
      title: '상품이 발송됐습니다!',
      body:  `${claim.productName} 발송 완료. ${trackingCarrier} ${trackingNumber}`,
      data:  { type: 'shipped', claimId }
    })
  } else if (status === 'delivered') {
    await sendToUser(claim.userId, {
      title: '배송 완료!',
      body:  `${claim.productName}이 배송 완료됐습니다. 잘 받으셨나요?`,
      data:  { type: 'delivered', claimId }
    })
  }
...
// ─────────────────────────────────────────────
// 5. 고가 상품 부정 참여 검증 (관리자 호출)
// ─────────────────────────────────────────────
export const verifyPremiumClaim = functions.https.onCall(async (data) => {
  const { claimId, verified, note } = data as {
    claimId: string; verified: boolean; note?: string
  }
  if (!claimId) throw new functions.https.HttpsError('invalid-argument', 'claimId 필수')
  ...
  const body  = verified
    ? `${claim.productName} 검증이 완료됐습니다. 마이페이지에서 배송지를 입력해주세요.`
    : `${claim.productName} 검증 결과: ${note ?? '부정 참여가 의심됩니다.'}`

  await sendToUser(claim.userId, {
    title, body,
    data: { type: 'verification_result', claimId, verified: String(verified) }
  })
...
export const applyInviteCode = functions.https.onCall(async (data) => {
  ...
  const mySnap = await db.ref(`users/${userId}`).once('value')
  const me     = mySnap.val()
  if (me?.inviteApplied) throw new functions.https.HttpsError('already-exists', '이미 초대 코드를 사용했습니다')
  ...
  if (inviterId === userId) throw new functions.https.HttpsError('invalid-argument', '자신의 코드는 사용 불가')

  await Promise.all([
    db.ref(`users/${userId}`).update({
      ticketCount:   admin.database.ServerValue.increment(3),
      inviteApplied: true
    }),
    db.ref(`users/${inviterId}`).update({
      ticketCount: admin.database.ServerValue.increment(3)
    })
  ])

  await sendToUser(inviterId, {
    title: '친구가 초대 코드를 사용했습니다!',
    body:  '도전권 3장이 지급됐습니다.',
    data:  { type: 'invite_reward' }
  })
...
async function sendToUser(
  ...
  try {
    await fcm.send({
      ...
      android: {
        priority: 'high',
        notification: { channelId: 'baekbunuil_main', sound: 'default' }
      }
    })
  } catch {
